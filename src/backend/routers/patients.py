...
def get_patient_report(patient_id: str, db: Session = Depends(get_session)):
    """Return a structured text and metadata report for clinician review."""
    patient = db.get(Patient, patient_id)
    if not patient:
        raise HTTPException(status_code=404, detail=f"Patient {patient_id} not found")
    ...
    completed = [s for s in sessions_list if s.status == SessionStatus.completed]
    ...
    ai_insight = None
    try:
        from ai_service import generate_patient_insight
        ai_insight = generate_patient_insight(patient, sessions_list)
    except Exception:
        ai_insight = None
    ...
    if ai_insight:
        ai_section = f"""
AI CLINICAL DECISION SUPPORT ({ai_insight.get('provider', 'watsonx-granite')})
  Status: {ai_insight.get('status')}
  Suggested Review: {ai_insight.get('suggested_review')}
"""
...
"""
    return {
        ...
        "age": patient.age,
        "diagnosis": patient.diagnosis,
        "start_date": str(patient.start_date),
        "planned_sessions_per_week": patient.planned_sessions_per_week,
        ...
        "recent_sessions": [TreatmentSessionRead.model_validate(s).model_dump() for s in sessions_list[:5]],
        "ai_insight": ai_insight,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "disclaimer": "Decision support only — clinician judgement required. AI output does not constitute a diagnosis, treatment recommendation, or prescription.",
    }
